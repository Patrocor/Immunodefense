(function () {
  ...
  // 3 caminos (F2) que convergen en Sepsis → MODS.
  //
  // F1 ─── DIS ─── F2 ─── F3 ─── SEPSIS ─── MODS
  //
  ...
  function handleBodyMapTap(x, y) {
    if (!state.bodyMap) return false;
    if (UI.bodyMapBtn && inRect(x, y, UI.bodyMapBtn)) {
      var cb = state.bodyMap.onContinue;
      state.bodyMap = null;
      UI.bodyMapBtn = null;
