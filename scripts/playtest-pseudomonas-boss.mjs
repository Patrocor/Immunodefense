...
const TMP = "/tmp/pseudo-boss-shots";
mkdirSync(ART, { recursive: true });
mkdirSync(TMP, { recursive: true });
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function placePseudo(shieldOn) {
  const g = window.__game;
  const st = g.state;
  const src = st._pseudoSrc;
  ...
  return {
    ok: true,
    shieldOn: !!shieldOn,
    clip: { x: Math.round(sx - 300), y: Math.round(sy - 220), width: 620, height: 440 },
  };
}

function spawnPseudo() {
  const g = window.__game;
  const st = g.state;
  st.showTitle = false;
  st.showIntro = false;
  ...
  ["hsv", "sepidermidis", "molluscum", "demodex", "cacnes", "saureus", "malassezia", "dermatofito", "neisseria", "hpv", "sarna", "leishmania", "candida", "bossPyogenes", "bossPseudomonas"].forEach((id) => {
  ...
  st.msgTimer = 0;
  g.step(1.2, 0.05);
  const def = window.ImmunoDefenseData.enemyDefs.bossPseudomonas;
  if (!def) return { ok: false, reason: "no bossPseudomonas def", screen: !!st.showTitle };
  st._pseudoSrc = { def, hp: def.hp, maxHp: def.hp, wobble: 0.5 };
...
const spawned = await page.evaluate(spawnPseudo);
console.log("Spawn:", spawned);
await page.mouse.move(16, 16);

const withCap = await page.evaluate(placePseudo, true);
console.log("Alginate on:", withCap);
await sleep(800);
await page.screenshot({ path: join(TMP, "pseudo_field.png") });
if (withCap.ok) {
  await page.screenshot({ path: join(TMP, "pseudo_cap.png"), clip: withCap.clip });
}

const bare = await page.evaluate(placePseudo, false);
console.log("Alginate off:", bare);
await sleep(600);
if (bare.ok) {
  await page.screenshot({ path: join(TMP, "pseudo_bare.png"), clip: bare.clip });
}
