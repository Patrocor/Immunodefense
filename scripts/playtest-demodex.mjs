...
await sleep(900);
await page.screenshot({ path: join(ART, "demodex_v3_pore_field.png") });
if (shown.ok) {
  await page.screenshot({ path: join(ART, "demodex_v3_pore_revealed.png"), clip: shown.clip });
}
...
await sleep(700);
if (hid.ok) {
  await page.screenshot({ path: join(ART, "demodex_v3_pore_cloaked.png"), clip: hid.clip });
}
